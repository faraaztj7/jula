...
class User_Login_Element extends Base {
    ...
    protected function render( $instance = [] ) {
        ...
        $title = ( $settings['form_title'] ? '<span>'.esc_html( $settings['form_title'] ).'</span>' : '' );
        $heighttitle = ( $settings['form_heightlight_title'] ? '<span class="htmove-heightlight">'.esc_html( $settings['form_heightlight_title'] ).'</span>' : '' );

        if( !empty($settings['form_heightlight_title']) ){
            $this->add_render_attribute( 'area_attr', 'class', 'htmove-form-title-pos-'.esc_attr($settings['form_heightlight_title_pos']) );
        }

        // Label Value
        $user_label = ( !empty( $settings['user_label'] ) ? esc_html( $settings['user_label'] ) : esc_html__( 'Username', 'moveaddons' ) );
        $user_placeholder = ( !empty( $settings['user_placeholder'] ) ? esc_html( $settings['user_placeholder'] ) : esc_html__('Username','moveaddons') );
        $pass_label = ( !empty( $settings['password_label'] ) ? esc_html( $settings['password_label'] ) : esc_html__('Password','moveaddons') );
        $pass_placeholder = ( !empty( $settings['password_placeholder'] ) ? esc_html( $settings['password_placeholder'] ) : esc_html__('Password','moveaddons') );
            ...
            <div <?php echo $this->get_render_attribute_string( 'area_attr' ); ?> >
                ...
                <?php
                    if ( is_user_logged_in() && !move_addons_get_elementor()->editor->is_edit_mode() ) {
                        ...
                        echo '<div class="htmove-user-login">' .sprintf( __( 'You are Logged in as %1$s (<a href="%2$s">%3$s</a>)', 'moveaddons' ), esc_html($current_user->display_name), wp_logout_url( $current_url ), esc_html__('Logout','moveaddons') ) .'</div></div>';
                        return;
                    }
                ?>
                ...
                <form id="htmove_login_form_<?php echo esc_attr( $id ); ?>" action="formloginaction" method="post">
                    ...
                    <div class="htmove-form-group htmove-mb-30">
                        ...
                        <?php
                            if( $settings['form_show_label'] == 'yes'){
                                echo sprintf('<label for="login_username-%1$s" class="htmove-form-label">%2$s</label>',esc_attr($id), esc_html( $user_label ) );
                            }
                        ?>
                    ...
                    <div class="htmove-form-group htmove-mb-20">
                        <?php
                            if( $settings['form_show_label'] == 'yes'){
                                echo sprintf('<label for="login_password-%1$s" class="htmove-form-label">%2$s</label>',esc_attr($id), esc_html($pass_label) );
                            }
                        ?>
                        <div class="htmove-form-control">
                            <input 
