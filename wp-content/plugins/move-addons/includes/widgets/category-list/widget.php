...
class Category_List_Element extends Base {
    ...
    protected function render( $instance = [] ) {
        ...
        $this->add_render_attribute( 'area_attr', 'class', 'htmove-category-list-'.esc_attr($settings['layout']) );

        if( $settings['list_icon_style'] != 'custom' ){
            $this->add_render_attribute( 'area_attr', 'class', 'htmove-category-list-icon-style-'.esc_attr($settings['list_icon_style']) );
        }
                ...
                echo '<ul>';
                    foreach ( $categories as $catkey => $cat ) {
                        ...
                        if( array_key_exists( $cat->slug, $customdata ) ){
                            ...
                            $badge = !empty( $customdata[$cat->slug]['badge'] ) ? '<span class="htmove-badge">'.esc_html($customdata[$cat->slug]['badge']).'</span>' : '';

                            echo sprintf('<li><a href="%1$s"><span>%2$s %3$s</span> <small>%4$s %5$s</small></a></li>', esc_url( $term_link ), $icon, esc_html__( $cat->name, 'moveaddons' ), esc_html($subtitle), $badge  );
