    ...
    protected function render( $instance = [] ) {
        ...
        if( $settings['videocontainer'] != 'self' ){
            ...
            if( !empty( $settings['video_image']['url'] ) ){
                $this->add_render_attribute( 'area_attr', 'style', 'background-image:url('.esc_attr($settings['video_image']['url']).')' );
            }

            $button = '';
            if( !empty( $settings['button_icon']['value'] ) ){
                $this->add_render_attribute( 'area_attr', 'class', 'htmove-button-position-'.esc_attr($settings['button_position']) );
                $button = move_addons_render_icon( $settings, 'button_icon', 'buttonicon' );
            }

            $title = ( !empty( $settings['title'] ) ? '<h2 class="htmove-video-title">'.esc_html($settings['title']).'</h2>' : '' );
            $subtitle = ( !empty( $settings['subtitle'] ) ? '<span class="htmove-video-sub-title">'.esc_html($settings['subtitle']).'</span>' : '' );
        ...
        if( $settings['videocontainer'] == 'self' ){
            ...
            $player_options_settings = [
                'videoURL'          => !empty( $settings['video_url'] ) ? esc_url($settings['video_url']) : 'https://www.youtube.com/watch?v=CDilI6jcpP4',
                ...
                <div class="htmove-video-content">
                    <?php
                        if( !empty( $settings['buttontext'] ) ){
                            echo sprintf('<a href="%1$s" class="%2$s">%3$s</a>',esc_url( $settings['video_url'] ),'htmove-video-btn htmove-video-popup',$button.$settings['buttontext'] );
                        }else{
