...
class Mailchimp_Element extends Base {
            ...
            if ( ! move_addons_is_option('htmove_userdata_list','mailchimpapi','value') ) {
                $this->add_control(
                    ...
                    [
                        ...
                        'raw' => wp_kses_post( '<p>Please Insert Mailchimp API Key from Here</p>(<a href="'.admin_url("admin.php?page=move-elementor#userdata").'" target="_blank">Click</a>)' ),
                    ]
                );
            }else{
                $this->add_control(
    ...
    protected function render( $instance = [] ) {
        ...
        $this->add_render_attribute( 'area_attr', 'class', 'htmove-mailchimp-form htmove-mailchimp-form-'.esc_attr($settings['style_type']) );

        $data_settings = [
            'mailchimpid'  => ( !empty( $settings['mailchimp_id'] ) ? $settings['mailchimp_id'] : Null ),
            'btntxt'  => ( !empty( $settings['submit_btn_text'] ) ? $settings['submit_btn_text'] : esc_html__('Subscribe Now','moveaddons') ),
            'loadingtxt'  => ( !empty( $settings['submit_btn_loading_text'] ) ? esc_html($settings['submit_btn_loading_text']) : Null ),
            'success_msg'  => ( !empty( $settings['success_msg'] ) ? esc_html($settings['success_msg']) : Null ),
            'error_msg'  => ( !empty( $settings['error_msg'] ) ? esc_html($settings['error_msg']) : Null )
        ];
            ...
            <form id="htmove-mailchimp-<?php echo $id; ?>" method="post">
                ...
                <div class="htmove-form-group">
                    <?php
                        if( $settings['show_label'] == 'yes' ){
                            echo '<label for="htmove-email-'.esc_attr($id).'" class="htmove-form-label">'.esc_html__( $settings['email_label'],'moveaddons' ).'</label>';
                        }
                    ?>
                    <div class="htmove-form-control">
                        ...
                        ?>
                        <input id="htmove-email-<?php echo esc_attr($id); ?>" name="email" type="email" placeholder="<?php echo esc_attr__( $settings['email_placeholder'], 'moveaddons' ); ?>" required />
                    </div>
                </div>
                ...
                <?php if( $settings['show_first_name'] == 'yes' ): ?>
                    <div class="htmove-form-group">
                        <?php
                            if( $settings['show_label'] == 'yes' ){
                                echo '<label for="htmove-first-name-'.esc_attr($id).'" class="htmove-form-label">'.esc_html__( $settings['first_name_label'],'moveaddons' ).'</label>';
                            }
                        ?>
                        <div class="htmove-form-control">
                            <?php 
                            ...
                            ?>
                            <input id="htmove-first-name-<?php echo esc_attr($id); ?>" 
                ...
                <?php if( $settings['show_last_name'] == 'yes' ): ?>
                    <div class="htmove-form-group">
                        <?php
                            if( $settings['show_label'] == 'yes' ){
                                echo '<label for="htmove-last-name-'.esc_attr($id).'" class="htmove-form-label">'.esc_html__( $settings['last_name_label'],'moveaddons' ).'</label>';
                            }
                        ?>
                        <div class="htmove-form-control">
                            <?php 
                            ...
                            ?>
                            <input id="htmove-last-name-<?php echo esc_attr($id); ?>" name="last_name" type="text" placeholder="<?php echo esc_attr__( $settings['last_name_placeholder'], 'moveaddons' ); ?>">
                        </div>
                    </div>
                <?php endif; ?>
