...
class Data_Table_Element extends Base {
    ...
    protected function render( $instance = [] ) {
        ...
        $this->add_render_attribute( 'area_attr', 'class', 'htmove-data-table htmove-data-table-'.esc_attr($settings['table_layout']) );
                ...
                <table class="<?php if( $settings['show_datatable_sorting'] == 'yes' ){ echo 'htmove-datatable-'.$id; } ?>">
                    ...
                    <?php for( $i = 0; $i < count( $table_tr ); $i++ ) : ?>
                        <tr>
                            ...
                            ?>
                                <td<?php echo $table_td[$j]['colspan'] > 1 ? ' colspan="'.$table_td[$j]['colspan'].'"' : ''; ?>><?php echo esc_html($table_td[$j]['title']); ?></td>
                            <?php endif; endfor; ?>
                        </tr>
                    <?php endfor;?>
                    </tbody>
